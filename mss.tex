...
\item  Optional fief-based routing, allowing comets to route directly by static IP or domain and lifting the assumption that sponsor chains must terminate at galaxies.
\end{enumerate}
\noindent \sloppy
...
\emph{Adapted from \textsc{uip}-XXX.}\footnote{The author would like to thank \patp{tinnus-napbus} and \patp{bonbud-macryg} for help documenting the original Groundwire codebase, and \patp{tondes-sitrym} and \patp{sarpen-laplux} for developing it.}
\end{abstract}
...
The remainder of this article recaps elliptic-curve context and the current asymmetric cryptosuite interface used by Ames, then details the tweaked-key construction and routing implications. A prototype exists in Groundwire’s forks of \lstinline[style=inlinecode]{urbit/urbit} and \lstinline[style=inlinecode]{urbit/vere} implementing \lstinline[style=inlinecode]{+fetch-comet-pki}, \lstinline[style=inlinecode]{+cric}, and \lstinline[style=inlinecode]{$fief}.
...
If Alice multiplies her private key by Bob's public key, Alice gets the same answer as if Bob multiplied his private key by Alice's public key. This allows the two parties to use that answer as a shared secret that only they know. In the \emph{Elliptic Curve Diffie-Hellman (\textsc{ecdh})} protocol, we take the $x$ coordinate of that shared secret and run it through a key derivation function (in Urbit's case, \textsc{sha}-256) to get a symmetric key which allows them to both encrypt and decrypt messages. Using this key, they can communicate privately.

Similarly, \emph{Elliptic Curve Digital Signature Algorithm (\textsc{ecdsa})} uses a more extensive algorithm to allow Alice to sign a message with her private key and for Bob to use his public key to verify that Alice did in fact sign it.
...
\begin{itemize}
  \item  \lstinline[style=inlinecode]{+scam}: scalar multiply. Takes a point and a scalar → returns scalar × point. This is the core elliptic curve operation. ward: point addition (Edwards curve formula). Needed for scalar multiplication by repeated doubling-and-adding.
  ...
  \item  \lstinline[style=inlinecode]{luck}: derive a keypair (public + private) from a seed. This is how comets/\-planets get Curve25519 keys.
...
\begin{itemize}
  \item  \lstinline[style=inlinecode]{seal}: encrypt to a peer. Runs \textsc{ecdh} with our encode secret + their encode pubkey, hashes with \textsc{sha}-256, signs with our auth secret, encrypts with \textsc{aes}-\textsc{siv}.
  ...
  \lstinline[style=inlinecode]{tear}: decrypt from a peer. \textsc{ecdh} with our enc secret + their enc pubkey, \textsc{sha}-256, \textsc{aes}-\textsc{siv} decrypt, then verify signature.
  ...
  \item  \lstinline[style=inlinecode]{pub}: concatenate encode pubkey + auth pubkey into one atom.
  \item  \lstinline[style=inlinecode]{sec}: concatenate encode secret + auth secret into one atom.
