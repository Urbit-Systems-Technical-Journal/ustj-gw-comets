...
If Alice multiplies her private key by Bob's public key, Alice gets the same answer as if Bob multiplied his private key by Alice's public key. This allows the two parties to use that answer as a shared secret that only they know. In the \emph{Elliptic Curve Diffie-Hellman (}\ssc{ecdh}\emph{)} protocol, we take the $x$ coordinate of that shared secret and run it through a key derivation function (in Urbit's case, \textsc{sha}-256) to get a symmetric key which allows them to both encrypt and decrypt messages. Using this key, they can communicate privately.

Similarly, \emph{Elliptic Curve Digital Signature Algorithm (}\ssc{ecdsa}\emph{)} uses a more extensive algorithm to allow Alice to sign a message with her private key and for Bob to use his public key to verify that Alice did in fact sign it.
...
\begin{itemize}
  ...
  \item  \lstinline[style=inlinecode]{+tear}: decrypt from a peer. \textsc{ecdh} with our enc secret plus their enc pubkey, \textsc{sha}-256, \textsc{aes}-\textsc{siv} decrypt, then verify signature.
...
In current Arvo (Listing~\ref{lst:arvo}), when a comet contacts another ship, that ship will ask the comet for an \emph{unecrypted self-attestation packet} to prove that the sender does in fact control the private key whose public counterpart was hashed into the comet's \lstinline[style=inlinecode]{@p}.
